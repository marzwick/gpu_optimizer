...
class GPUNetworkOptimizer:
    ...
    def _calculate_water_usage(self, gpu_power_watts, lat, lon):
        ...
        water_usage = power_kw * base_rate
        
        return water_usage, climate_zone
    ...
    def _calculate_total_cost(self, row, hours):
        ...
        water_usage_lph, climate_zone = self._calculate_water_usage(
            row['gpu_power_watts'], 
            row['latitude'], 
            row['longitude']
        )
        water_cost = self._calculate_water_cost(water_usage_lph, hours)
        ...
        return {
            ...
            'water_usage_liters': water_usage_lph * hours,
    ...
    def create_sustainability_report(self, profile='water_conscious', hours=24):
        ...
        report = {
            ...
            'environmental_impact': {
                'water_usage_liters': round(costs['water_usage_liters'], 1),
                'water_usage_per_hour': round(costs['water_usage_liters'] / hours, 1),
                'climate_zone': costs['climate_zone'],
                'sustainability_grade': self._calculate_sustainability_grade(costs)
            }
        }
        
        return report
    
    def _calculate_sustainability_grade(self, costs):
        """Assign sustainability grade based on water usage"""
        water_per_hour = costs['water_usage_liters'] / 24  # Assuming 24 hours for normalization
        
        if water_per_hour < 50: return 'A+'
        elif water_per_hour < 100: return 'A'
        elif water_per_hour < 200: return 'B'
        elif water_per_hour < 350: return 'C'
        else: return 'D'
    ...
    def create_map(self, profile='ultra_latency', hours=24, show_climate_zones=False):
        ...
        Water Usage: {costs['water_usage_liters']:.1f}L<br>
        Sustainability: {self._calculate_sustainability_grade(costs)}
        """
        
        # Color code by sustainability grade
        grade = self._calculate_sustainability_grade(costs)
        marker_color = {'A+': 'green', 'A': 'lightgreen', 'B': 'orange', 'C': 'red', 'D': 'darkred'}[grade]
